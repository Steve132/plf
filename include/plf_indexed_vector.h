...
{
private:
	typedef std::vector<unsigned int, typename std::allocator_traits<allocator_type>::template rebind_alloc<unsigned int>> index_vector;
...
public:
	...
	{
		elements.push_back(element);
		return element_indexes.insert(position, current_back_index++);
	}
